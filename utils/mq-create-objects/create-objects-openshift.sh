...
if $SSL_ENABLED
then
  kubectl apply -f ../../dependencies/mq-cluster-openshift.yaml -n $RABBIT_NAMESPACE
else
  kubectl apply -f ../../dependencies/mq-cluster-openshift_NOTLS.yaml -n $RABBIT_NAMESPACE
fi
